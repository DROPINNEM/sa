...
user.guild.channels.cache.get(kanallar.Tag_Log).send(new MessageEmbed().setAuthor(`Tag Alındı`).setDescription(`${user} adlı üye \`${settings.sunucuTag}\` tagımızı alarak ailemize katıldı!`).setColor("GREEN").setTimestamp().setFooter(`DROPİNNEM ♥ PANKART`));
...
user.guild.channels.cache.get(kanallar.Tag_Log).send(new MessageEmbed().setAuthor(`Tag Bırakıldı`).setDescription(`${user} adlı üye \`${settings.sunucuTag}\` tagımızı bıraktı.`).setColor("RED").setTimestamp().setFooter(`DROPİNNEM ♥ PANKART`));
}
