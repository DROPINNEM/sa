...
if(member.user.username.includes(settings.sunucuTag)){
member.roles.add(roller.TagRolü)
member.guild.channels.cache.get(kanallar.Tag_Log).send(new MessageEmbed().setAuthor(`Taglı Geldi`).setDescription(`${member} adlı üye \`${settings.sunucuTag}\` tagımızı alarak gelmiş.`).setColor("GREEN").setTimestamp().setFooter(`DROPİNNEM ♥ PANKART`));
}
}
