...
module.exports = {
conf: {name: 'isim', aliases: ["isim-değiş"], help: "!erkek @DROPİNNEM/ID İsim Yaş"},
...
member.setNickname(`${ServerDisplayName}`);
  ...
  let isimler = kdb.get(`kullanici.${member.id}.isimler`);
   ...
   if(isimler.length < 2) return message.channel.send(embed.setDescription(`${member} adlı üyenin ismi "${ServerDisplayName}" olarak güncellendi.`).setColor(client.generalColor())).then(x => x.delete({timeout:15000}), message.react(SEmoji.Onayla)) 

message.channel.send(embed.setDescription(
            `${member}(\`${member.id}\` adlı kullanıcı \n ${message.author} (\`${message.author.id}\`)  tarafından isimi değiştirildi \n\n **Kullanıcının önceki kayıtlardan isimleri bulundu**\n ${strigalamacool}`
          ).setColor(client.generalColor())).then(x => x.delete({timeout:15000}), message.react(SEmoji.Onayla)) 
