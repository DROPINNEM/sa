...
module.exports = {
conf: {name: 'kayıtsız', aliases: ["unregister"], help: "!kayıtsız @DROPİNNEM/ID Sebep"},
...
message.channel.send(embed.setDescription( `${member}(\`${member.id}\` adlı kullanıcı \n ${message.author} (\`${message.author.id}\`)  tarafından kayıtsıza atıldı `).setColor(client.manColor())).then(x => x.delete({timeout:15000}), message.react(SEmoji.Onayla)) 
...
member.setNickname(member.user.username)
  ...
  member.setNickname(member.user.username)
  member.setNickname(member.user.username)
 member.roles.add(SRol.KayıtsızRolü)
   member.roles.add(SRol.KayıtsızRolü)
   member.roles.add(SRol.KayıtsızRolü)
