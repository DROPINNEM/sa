...
let page = 1;
let sayi = 1;
let isimlerinformation = isimlerData.length > 0 ? isimlerData.map(x => `\`${sayi++}.\` **${x.DisplayName}** \`(${x.Cinsiyet})\` \`${x.Date}\` [${message.author}]`) : `${member} adlı kullanıcının herhangi bir isimler kayıtı yok.`
...
${isimlerinformation.slice(page == 1 ? 0 : page * 10 - 10, page * 10).join("\n")}`).setFooter(`DROPİNNEM ♥ PANKART`)).then(message.react(SEmoji.Onayla))
...
${dataNext}`).setFooter(`DROPİNNEM ♥ PANKART`))
}
...
${dataEx}`).setFooter(`DROPİNNEM ♥ PANKART`))
}
if (react.emoji.name == "🔴") {
isimlerListe.delete();
collector.stop();}})
